...
```bash
# Publish the dist/ folder using the platform command documented by game-deploy.
# Before executing, verify the command exists locally or is provided by the project.
```
...
```bash
# Publish dist/ with the existing slug using the verified here.now command.
```
...
```bash
# Deploy dist/ to gh-pages branch through a repo-local deploy script.
npm run deploy
```
