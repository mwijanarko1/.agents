...
- **`monetize-game`** — Play.fun (OpenGameProtocol) monetization integration
...
5. **Install Playwright and Chromium** — Playwright is required for runtime verification and the iterate loop:
   1. Check if Playwright is available through the project test script or installed package binary.
   2. If that fails, check `node_modules/.bin/playwright --version`
   3. If neither works, run `npm install -D @playwright/test` explicitly
   4. Then install the browser binary with the repo-local Playwright package binary.
