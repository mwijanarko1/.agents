...
Use `~/Documents/llm-wiki` as the shared Obsidian-style Codex memory vault for durable context outside any single repo or thread. A convenience symlink exists at `~/.agents/state/memory/llm-wiki`, and operating notes live in `~/.agents/state/memory/CODEX_OBSIDIAN_MEMORY.md`.

When a task depends on personal/project memory, wiki lookup, Codex memory, or durable context, read the vault's `AGENTS.md` and `index.md` first. Prefer `python3 ~/.agents/scripts/agent_wiki.py context "<query>"` or `python3 ~/.agents/scripts/agent_wiki.py search "<query>"` for quick retrieval. Keep `raw/` immutable, update relevant `wiki/` pages for durable knowledge, and append audit notes to `log.md` rather than rewriting history.
...
2. **Disclose** — In the first substantive assistant message of the thread (or at the start of a one-shot subagent run), list which skills you actually loaded, using each skill’s directory name only (for example `coding-standards`, `frontend-web-development`). Skill disclosure should be one short sentence.
...
| Imported (ECC curated) | continuous-learning-v2, tdd-workflow, verification-loop, eval-harness, ai-regression-testing, search-first, documentation-lookup, strategic-compact, context-budget, gateguard, architecture-decision-records, skill-stocktake |
